...
flowchart TB
    ...
    Task -.->|"AssumeRole → AWS API ReadOnly<br/>※ 拡張オプション"| Members
    Task -->|ECR pull| ECR
...
| 機能                | 概要                                                                                                                            |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| **クロスアカウント read** | 組織内の他アカウント (dev/stg/prod 等) のリソースを AI チャットから横断検索。CUR は集約済みのため「コスト分析」自体は単一アカウントで動くが、リソース実態確認 (EC2 describe 等) は AssumeRole が要る |
