...
hakumei は AWS 標準ツールの置き換えではなく、**人が個別のコンソールを巡回して読み解いていた情報を、CUR (請求実績の生データ) と AWS API (実リソース) で裏取りした上で 1 ページに自動集約**するツールです。

- **1 ページに自動集約**: Cost Explorer / Cost Optimization Hub / Trusted Advisor / 各サービスコンソールを人が巡回する代わりに、コストレポート・削減推奨・AI サマリが 1 画面に揃う
- **実態ベース**: 表示する数字は CUR (確定した請求実績) と AWS API (実リソースの現在の状態) の突合結果。推定値ではなく実績
- **推奨の解像度**: 標準ツールの削減推奨 (メトリクスベースの概算) を実コスト・実リソースで裏取りし、妥当性 (妥当 / 条件付き / 疑わしい) を AI が判定
