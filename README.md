...
再開時の通常タスク数はデフォルト 1 (`scheduled_scaling_min_task_count`) です。可用性・負荷分散のため平日は複数タスクを維持したい場合は `scheduled_scaling_min_task_count = 2` のように指定してください (下記トレードオフ参照)。負荷時にスケールアウトする上限は `scheduled_scaling_max_task_count` (デフォルト 20) で別途指定します。
...
- **再開時の最小タスク数 (`scheduled_scaling_min_task_count`、デフォルト 1) を 2 以上にすると、上記のキャッシュ消失・ウォームアップの遅延・固定費がほぼ min 倍になります**。可用性・負荷分散のため平日稼働中は複数タスクを維持したい場合のみ指定してください (#738)
- **自動更新 (`auto_update_enabled`、§7) と組み合わせても安全です**: デフォルトの自動更新スケジュール (毎日 JST 04:00) は、上記のようなよくある停止時間帯 (夜間〜早朝) とちょうど重なりますが、競合しません。Express Mode の `update-express-gateway-service` は「タスク数 (`scalingTarget`)」と「イメージ (`primaryContainer`)」を独立したパラメータとして扱うため、自動更新 Lambda がイメージだけを更新してもタスク数には影響しません。停止中 (タスク数 0) にイメージが更新されても新規タスクは起動されず、次の再開スケジュールで新しいイメージのタスクがそのまま立ち上がります (デモ環境で実機検証済み、2026-08-27)
