...
- AI チャット (自然言語で「先月の RDS が高い理由は?」と聞ける)
...
flowchart TB
    ...
    Task -.->|"AssumeRole → AWS API ReadOnly<br/>※ 任意 (§8)"| Members
    Task -->|ECR pull| ECR
...
- **破線**: 任意オプション (デフォルトでは作らない。§8 の手順で有効化)

メンバーアカウントから管理アカウントへの CUR 集約は AWS Billing が自動で行うため、hakumei は管理アカウントの CUR S3 を読むだけで組織全体のコストが見えます。各メンバーアカウントの実リソース (EC2 describe など) を AI チャットから直接調べたい場合だけ、AssumeRole の経路を別途構築します (§8)。
...
| 自動更新   | Lambda + EventBridge Scheduler + 専用 IAM ロール 2 種 + Lambda Logs | `auto_update_enabled = true` (デフォルト) のときのみ。§6 参照                       |
...
- Lambda を手動 invoke して挙動を確認 (§6「手動で即時更新する」参照)
...
### 8.1 クロスアカウント read (メンバーアカウント実態確認)

組織内の他アカウント (dev/stg/prod 等) のリソース実態 (EC2 describe 等) を AI チャットから横断で読めるようにします。CUR は管理アカウントに集約済みのため「コスト分析」自体は本手順なしで組織全体をカバーしますが、AI チャットがメンバーアカウントの実リソースを調べるには AssumeRole の経路が必要です。
...
新版は ECR Public の `:latest` に都度 push されるため、デフォルト設定 (`auto_update_enabled = true`) であれば毎日 JST 04:00 に自動反映されます (§6)。
